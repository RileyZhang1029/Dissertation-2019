...
from ..util import export_model_data, set_hashes, log, prints, split_string
...
def match(
    ...
    if resume and dataset is not None and dataset in DB:
        ...
        log(
            "RECIPE: Updating PatternMatcher with {} examples from dataset {}".format(
                len(existing), dataset
            )
        )
    ...
    if label is not None and patterns is None:
        ...
        for l in label:
            if not model.has_label(l):
                prints(
                    "Can't find label '{}' in model {}".format(l, spacy_model),
...
def manual(
    dataset, spacy_model, source=None, api=None, loader=None, label=None, exclude=None
):
    """
...
@recipe(
    ...
    dataset=recipe_args["dataset"],
    silver_datasets=(
        "Comma-separated datasets to convert",
        "positional",
        None,
        split_string,
    ),
...
def silver_to_gold(dataset, silver_datasets, spacy_model, label=None):
    """
    ...
    DB = connect()
    silver_data = []
    for set_id in silver_datasets:
        if set_id not in DB:
            prints(
                "Can't find dataset '{}' in database. Did you specify the "
                "input datasets correctly?".format(set_id),
                error=True,
                exits=1,
            )
        examples = DB.get_dataset(set_id)
        silver_data += examples
    log(
        "RECIPE: Loaded {} examples from {} dataset(s)".format(
            len(silver_data), len(silver_datasets)
        )
    )
    ...
    def make_stream():
        ...
        # make_best will always use all labels in the model, so we need to
...
def evaluate_prf(test_examples, nlp):
    tp = 0.0
    fp = 0.0
    fn = 0.0
...
def batch_train(
    ...
    if "sentencizer" not in nlp.pipe_names:
        ...
        log(
            "RECIPE: Added sentence boundary detector to model pipeline", nlp.pipe_names
        )
    ...
    if eval_id:
        ...
        print_(
            "Using {}% of accept/reject examples ({}) for evaluation".format(
    ...
    if other_pipes:
        ...
        disabled = None
    log(
        "RECIPE: Initialised EntityRecognizer with model {}".format(input_model),
        model.nlp.meta,
    )
    if not unsegmented:
    ...
    log(
        "RECIPE: Calculated baseline from evaluation examples "
    ...
    for i in range(n_iter):
        ...
        print_("precision:", scores[0], "recall:", scores[1], "fscore:", scores[2])
...
def print_best(dataset, spacy_model, pretty=False):
    ...
    log(
        "RECIPE: Initialised EntityRecognizer with model {}".format(spacy_model),
        model.nlp.meta,
    )
...
def pretty_print_stream(spacy_model, source=None, api=None, loader=None, label=""):
    """
...
def pretty_print_dataset(dataset):  # pragma: no cover
    ...
    if not examples:
        raise ValueError("Can't load '{}' from database {}".format(dataset, DB.db_name))
...
@recipe(
    ...
    spacy_model=(
        "Optional spaCy model for tokenization, required for " "BILUO mode",
        "option",
        "sm",
        str,
    ),
...
def gold_to_spacy(dataset, output_file=None, spacy_model=None, biluo=False):
    ...
    for eg in examples:
        entities = [
            (span["start"], span["end"], span["label"]) for span in eg.get("spans", [])
        ]
        if biluo:
            doc = nlp(eg["text"])
...
@recipe(
    "ner.iob-to-gold",
    input_file=(
        "Path to IOB or IOB2 formatted NER annotations",
        "positional",
        None,
        Path,
    ),
